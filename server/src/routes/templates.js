...
templatesRouter.get('/', async (req, res) => {
  ...
  if (isDriveConfigured() && folderId) {
    try {
      ...
      for (const f of files) {
        await upsert.run({ name: f.name, drive_file_id: f.id, drive_link: f.webViewLink || null });
      }
    } catch (err) {
  ...
  const templates = await db.prepare('SELECT * FROM templates ORDER BY name COLLATE NOCASE').all();
...
templatesRouter.post('/', async (req, res) => {
  ...
  const template = await db.prepare('SELECT * FROM templates WHERE id = ?').get(result.lastInsertRowid);

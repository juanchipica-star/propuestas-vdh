...
// propuesta historica de ejemplo, solo si la tabla de plantillas esta vacia. Se corre en
// cada arranque: es necesario porque en Vercel la base vive en /tmp y se reinicia vacia
...
export function seedTemplatesIfEmpty() {
  ...
  if (fs.existsSync(seedSourcePath)) {
    const clientId = db.prepare('INSERT INTO clients (name) VALUES (?)').run(SEED_PROPOSAL.clientName)
      .lastInsertRowid;
    const template = db
